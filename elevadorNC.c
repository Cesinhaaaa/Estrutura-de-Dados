...
//printf("\ni: %d, j: %d\n", i, j); system("pause");
                        if (j > 0) {
//printf("\nentrou no j > 0\n"); system("pause");
                            if (elevatorBank[i][j - 1].useBlocked) {
                                elevatorBank[i][j - 1].useBlocked = false;
                                elevatorBank[i][j - 1].direction = NONE;
                                elevatorBank[i][j - 1].originalStop = NONE;
                            }
                        }
                        
                        if (j < ELEVATORPERCHAIN - 1) {
//printf("\nentrou no j < %d\n", ELEVATORPERCHAIN - 1); system("pause");
                            if (elevatorBank[i][j + 1].useBlocked) {
                                elevatorBank[i][j + 1].useBlocked = false;
                                elevatorBank[i][j + 1].direction = NONE;
                                elevatorBank[i][j + 1].originalStop = NONE;
                            }
                        } else {//printf("\nNao entrou em nenhum\n"); system("pause");
                        }
                    }

                    if (elevator->fetching) {
                        elevator->fetching = false;
                        elevator->direction = ((elevator->currentFloor > elevator->originalStop) ? DOWN : UP);
                    }
                } else {
                    elevator->currentFloor += elevator->direction;
                }
            } else if (elevator->useBlocked) {
                if (elevator->direction == UP && elevator->currentFloor < FLOORS + 3)
                    elevator->currentFloor += elevator->direction;
                if (elevator->direction == DOWN && elevator->currentFloor > -2)
                    elevator->currentFloor += elevator->direction;
            }
        }
    }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if (!(aux->elevatorNum == 0 && elevator->direction == DOWN) && !(aux->elevatorNum == 5 && elevator->direction == UP)) {
            ...
            int end = (elevator->lastFloor) ? elevator->lastFloor->floorToStop : requestFloor;
            int topEnd = (topElevator->lastFloor) ? topElevator->lastFloor->floorToStop : 0;
            int bottomEnd = (bottomElevator->lastFloor) ? bottomElevator->lastFloor->floorToStop : 0;

            int count = stopCount(elevator->listOfStops);
            int topCount = stopCount(topElevator->listOfStops);
            int bottomCount = stopCount(bottomElevator->listOfStops);
