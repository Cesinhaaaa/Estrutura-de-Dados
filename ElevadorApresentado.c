        ...
        for (int j = 0; j < ELEVATORPERCHAIN; j++) {
            ...
            if (elevator->inUse) {
                if (elevator->currentFloor == elevator->listOfStops->floorToStop) { // atenção ao acesso de um ponteiro nulo
                    ...
                    int k = 1;
                    if (elevator->inUse == false) {
                        while (j - k >= 0) {
                            if (elevatorBank[i][j - k].useBlocked) {
                                elevatorBank[i][j - k].useBlocked = false;
                                elevatorBank[i][j - k].direction = NONE;
                                elevatorBank[i][j - k].originalStop = NONE;
                                elevatorBank[i][j - k].currentFloor = elevator->currentFloor - k;
                            }
                            k++;
                        }
                        
                        k = 1;
                        while (j + k <= ELEVATORPERCHAIN - 1) {
                            if (elevatorBank[i][j + k].useBlocked) {
                                elevatorBank[i][j + k].useBlocked = false;
                                elevatorBank[i][j + k].direction = NONE;
                                elevatorBank[i][j + k].originalStop = NONE;
            ...
            } else if (elevator->useBlocked) {
                // int k = 1;
                // while (elevatorBank[i][j - k].useBlocked || j - k >= 0) {
                //     elevatorBank[i][j - k].currentFloor += elevatorBank[i][j - k].direction;
                    
                //     k++;
                // }
                        
                // k = 1;
                // while (elevatorBank[i][j + k].useBlocked || j + k <= ELEVATORPERCHAIN - 1) {
                //     elevatorBank[i][j + k].currentFloor += elevatorBank[i][j + k].direction;
                ...
                //     int k = 1;
                //     if (j - k > 0) {
                //         if (elevatorBank[i][j - k].currentFloor + 1 == elevator->currentFloor)
                //             elevatorBank[i][j - k].currentFloor += DOWN;
                ...
                //     int k = 1;
                //     if (j - k < ELEVATORPERCHAIN - 1) {
                //         if (elevatorBank[i][j + k].currentFloor - 1 == elevator->currentFloor)
                //             elevatorBank[i][j + k].currentFloor += UP;
                //     }
                // }


                // if (elevator->direction == UP && elevator->currentFloor < FLOORS + 4)
                //     elevator->currentFloor += elevator->direction;
                // if (elevator->direction == DOWN && elevator->currentFloor > -4)
                //     elevator->currentFloor += elevator->direction;
            }
        }
    }
        ...
        for (int i = 0; i < ELEVATORCHAINS; i++) {
            ...
            if (elevator.inUse)
                if (elevator.listOfStops != NULL && elevator.currentFloor == elevator.listOfStops->floorToStop)
                    ...
                    printf(" %.3d -> %.3d", elevator.currentFloor, elevator.lastFloor->floorToStop);
            else if (elevator.useBlocked)
                printf(" <Bloqueado>");
            else
...
void initElevatorBank (Elevator elevatorBank[ELEVATORCHAINS][ELEVATORPERCHAIN]) {
    ...
    for (int i = 0; i < ELEVATORCHAINS; i++) {
        ...
        for (int j = 0; j < ELEVATORPERCHAIN; j++) {
            ...
            elevatorBank[i][j].useBlocked = false;
            elevatorBank[i][j].originalStop = NONE;
            elevatorBank[i][j].direction = NONE;
...
int stopCount (List *head) {
    if (head == NULL)
        return 0;
    else
        return 1 + stopCount(head->next);
}
...
void request (Elevator elevatorBank[ELEVATORCHAINS][ELEVATORPERCHAIN], int requestFloor, int requestDestiny) {
    ...
    while (aux) {
        ...
        originalDirection = elevator->direction;
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        if (elevator->direction == 0)
            if (elevator->listOfStops)
                elevator->direction = ((elevator->currentFloor > requestFloor) ? DOWN : UP);
            else if (elevator->currentFloor > requestFloor)
                elevator->direction = DOWN;
            else if (elevator->currentFloor < requestFloor)
                elevator->direction = UP;

        if (!(aux->elevatorNum == 0 && elevator->direction == DOWN) && !(aux->elevatorNum == 5 && elevator->direction == UP)) {
            
            Elevator *topElevator = &elevatorBank[aux->chain][aux->elevatorNum + 1];
            Elevator *bottomElevator = &elevatorBank[aux->chain][aux->elevatorNum - 1];

            int end = (elevator->lastFloor) ? elevator->lastFloor->floorToStop : requestDestiny;
            int topEnd = (topElevator->lastFloor) ? topElevator->lastFloor->floorToStop : topElevator->currentFloor;
            int bottomEnd = (bottomElevator->lastFloor) ? bottomElevator->lastFloor->floorToStop : bottomElevator->currentFloor;

            int count = 0;//stopCount(elevator->listOfStops);
            int topCount = 0;//stopCount(topElevator->listOfStops);
            int bottomCount = 0;//stopCount(bottomElevator->listOfStops);
                
            if (elevator->direction == DOWN) {
                if (bottomElevator->inUse) {
                    if (end - count < bottomEnd - bottomCount)
                        hasCollision = true;
                } else { // bloqueia todos os elevadores necessarios
                    int k = 1;
                    //printf("\nentrou no if down\n");
                    while (aux->elevatorNum - k >= 0) {
                        bottomElevator = &elevatorBank[aux->chain][aux->elevatorNum - k];
                        bottomCount = stopCount(bottomElevator->listOfStops);
                        bottomEnd = (bottomElevator->lastFloor) ? bottomElevator->lastFloor->floorToStop : bottomElevator->currentFloor;
            //printf("\nend - count: %d /// bottomEnd - bottomCount: %d", end - count, bottomEnd - bottomCount);
                        if (end - count <= bottomEnd - bottomCount) {
                            //printf("\nentrou no if de block\n");
                            bottomElevator->useBlocked = true;
                            bottomElevator->direction = DOWN;
                            bottomElevator->originalStop = end - 1;
                        }
                        k++; //getchar();
                    }
                }
            } else if (elevator->direction == UP) {
                if (topElevator->inUse) {
                    if (end - count > topEnd - topCount)
                        hasCollision = true;
                } else { // bloqueia todos os elevadores necessarios
                    int k = 1;
                    while (aux->elevatorNum + k <= ELEVATORPERCHAIN - 1) {
                        topElevator = &elevatorBank[aux->chain][aux->elevatorNum + k];
                        topCount = stopCount(topElevator->listOfStops);
                        topEnd = (topElevator->lastFloor) ? topElevator->lastFloor->floorToStop : topElevator->currentFloor;

                        if (end - count >= topEnd - topCount) {
                            topElevator->useBlocked = true;
                            topElevator->direction = UP;
                            topElevator->originalStop = end + 1;
                        }
                        k++;
                    }
                }
        ...
        elevator->direction = originalDirection;
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        if (!hasCollision && !elevator->useBlocked) {
            if (elevator->inUse) {
                if (elevator->direction == UP) {
    ...
    /*n linear*/} else {
                    ...
                    List *stop1 = malloc(sizeof(List));
                    List *stop2 = malloc(sizeof(List));

                    stop1->floorToStop = requestFloor;
                    stop2->floorToStop = requestDestiny;

                    stop1->next = stop2;
                    stop2->next = NULL;

                    stop1->boardingFloor = true;
                    stop2->boardingFloor = false;

                    elevator->listOfStops = stop1;
                    elevator->lastFloor = stop2;
